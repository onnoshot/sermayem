  ...
  for (const tx of completed) {
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        name: tx.source?.name ?? "Diğer Gider",
        emoji: tx.source?.emoji ?? "💸",
        color: tx.source?.color ?? "#EF4444",
        total: 0,
        count: 0,
        percentage: 0,
      })
  ...
  return (
    <StaggerChildren>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/40">
                Toplam: <span className="text-red-400 font-semibold tabular-nums">{formatCurrency(total, currency)}</span>
              </p>
            </div>
          </div>
      ...
      {stats.length > 1 && (
        <StaggerItem>
          <div className="mb-5">
            <CategoryBreakdown stats={stats} total={total} currency={currency} type="expense" />
          </div>
        </StaggerItem>
      )}
