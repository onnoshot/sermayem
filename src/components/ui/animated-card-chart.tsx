...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-[356px] overflow-hidden rounded-xl border border-[var(--c-border)] bg-[var(--c-surface)] shadow-sm",
        className
      )}
      {...props}
    />
...
export function CardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn(
        "flex flex-col space-y-1.5 border-t border-[var(--c-border)] p-4",
        className
      )}
      {...props}
    />
...
export function CardTitle({ className, ...props }: CardTitleProps) {
  return (
    <h3
      className={cn(
        "text-lg font-semibold leading-none tracking-tight text-[var(--c-text)]",
        className
      )}
      {...props}
    />
...
export function CardDescription({ className, ...props }: CardDescriptionProps) {
  return (
    <p
      className={cn("text-sm text-[var(--c-muted)]", className)}
      {...props}
    />
  );
}
...
const EllipseGradient: React.FC<{ color: string }> = ({ color }) => (
  <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
    <svg width="356" height="196" viewBox="0 0 356 180" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs>
        <radialGradient id="paint0_radial_12_207" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse"
...
const Layer1: React.FC<LayerProps> = ({ color, secondaryColor }) => (
  ...
  >
    <div className="flex shrink-0 items-center rounded-full border border-[var(--c-border)] bg-white/[0.06] px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
      <div className="h-1.5 w-1.5 rounded-full bg-[var(--color)]" />
      <span className="ml-1 text-[10px] text-[var(--c-text)]">+15,2%</span>
    </div>
    <div className="flex shrink-0 items-center rounded-full border border-[var(--c-border)] bg-white/[0.06] px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
      <div className="h-1.5 w-1.5 rounded-full bg-[var(--secondary-color)]" />
      <span className="ml-1 text-[10px] text-[var(--c-text)]">+18,7%</span>
    </div>
  </div>
);
...
const Layer2: React.FC<{ color: string }> = ({ color }) => (
  <div className="group relative h-full w-[356px]" style={{ "--color": color } as React.CSSProperties}>
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[7] flex w-[356px] translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
      <div className="ease-[cubic-bezier(0.6,0,1)] rounded-md border border-[var(--c-border)] bg-white/[0.06] p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100">
        <div className="flex items-center gap-2">
          <div className="h-2 w-2 shrink-0 rounded-full bg-[var(--color)]" />
          <p className="text-xs text-[var(--c-text)]">Finansal Veri Görselleştirme</p>
        </div>
        <p className="text-xs text-[var(--c-muted)]">Gelir ve gider analizleri.</p>
      </div>
    </div>
  </div>
);
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  ...
  return (
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[8] flex h-[180px] w-[356px] items-center justify-center text-white/[0.12] transition-transform duration-500 group-hover/animated-card:scale-150">
